...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Hi, {0.first_name}! Please select a use case!'.format(message.from_user),
                     reply_markup=markup)
...
def bot_message(message):
    ...
    if message.chat.type == 'private':
        ...
        elif MESSAGE_STATE == STATE_CONSTS["waiting"]:
            print("got here!")
            print(message.text)
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            message_text = ""
            message_text += "Your request is: " + message.text
            message_text += "\n\nFor possible vulnerabilities on " + message.text + " during the last three month please visit the next URL: \nhttps://nvd.nist.gov/vuln/search/results?form_type=Basic&results_type=overview&query=" + message_text + "&search_type=all"
            MESSAGE_STATE = STATE_CONSTS["start"]
            bot.send_message(message.chat.id, message_text, reply_markup=markup)
